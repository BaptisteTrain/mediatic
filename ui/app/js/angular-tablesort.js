...
tableSortModule.directive( 'tsRepeat', ['$compile', '$interpolate', function($compile, $interpolate) {
    return {
        ...
        link: function(scope, element, attrs, tsWrapperCtrl) {
            ...
            var repeatExprRegex = /^\s*([\s\S]+?)\s+in\s+([\s\S]+?)(\s+track\s+by\s+[\s\S]+?)?(\s+as\s+[\s\S]+?)?\s*$/;
            var trackByMatch = repeatExpr.match(/\s+track\s+by\s+(\s+as\s+[\s\S]+?)?\S+?\.(\S+)/);
            var asMatch = repeatExpr.match(/\s+as\s+\S+?\.(\S+)/);
            ...
            if (repeatExpr.search(/tablesort/) !== -1) {
                ...
                repeatExpr = repeatExpr.replace(repeatExprRegex, '$1 in $2 | ' + tsExpr + '$3' + '$4');
            }
