...
public class UserDAO extends GenericDAO<User> {
	...
	public List<User> findUsersByIdPartial(String id) {
		...
		DataBaseHelper.beginTx(em);
		TypedQuery<User> query = em.createQuery("SELECT u " + "FROM User u " + "WHERE u.identifier LIKE :id",
				User.class);
	...
	public List<User> findUserByName(String lastname, String firstname) {
		...
		TypedQuery<User> query = em.createQuery("SELECT u " + "FROM User u "
				+ "WHERE upper(u.person.lastname) LIKE :lastname " + "AND upper(u.person.firstname) LIKE :firstname",
				User.class);
	...
	public List<User> findUserByIdOrNames(String id, String lastname, String firstname) {
		EntityManager em = DataBaseHelper.getEntityManager();
		DataBaseHelper.beginTx(em);
		TypedQuery<User> query = em.createQuery("SELECT u " + "FROM User u " + "WHERE u.person.lastname LIKE :lastname "
				+ "OR u.person.firstname LIKE :firstname " + "OR u.identifier LIKE :id", User.class);
		query.setParameter("id", id + "%");
